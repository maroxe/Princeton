...
@
<<lasso, dev='png', fig.height=5, fig.cap='lasso', fig.pos='h!', cache=TRUE>>=
plot.path(alpha = 0)
@

<<ridge, dev='png', fig.height=5, fig.cap='ridge', fig.pos='h!', cache=TRUE>>=
plot.path(alpha = 1)
@ 

\item[b)]
<<cv, fig.show='hold', cache=TRUE>>=
...
r2.cv.lasso <- evaluate.on.test(alpha=0)
r2.cv.ridge <- evaluate.on.test(alpha=1)
@
$R^2$ error:
\[
  \begin{array}{c|c}
...
The following trains a random forest 
